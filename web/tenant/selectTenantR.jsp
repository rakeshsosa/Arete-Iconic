    ...
    <body>
        ...
        <script>
            function test(val1, val2, val3, val4, val5, val6, val7, val8, val9, val10, val11, val12, val13, val14, val15) {
                ...
                window.opener.document.getElementById('buildingNo').value = val4;
                window.opener.document.getElementById('blockNo').value = val5;
                window.opener.document.getElementById('streetNo').value = val6;
                window.opener.document.getElementById('plotNo').value = val7;
                window.opener.document.getElementById('rentAmount').value = val8;
                window.opener.document.getElementById('billCycle').value = val9;
                window.opener.document.getElementById('subassetCode').value = val10;
                window.opener.document.getElementById('flotNo').value = val11;
                window.opener.document.getElementById('assetCode').value = val12;
                window.opener.document.getElementById('contractCode').value = val13;
                window.opener.document.getElementById('startsFrom').value = val14;
                window.opener.document.getElementById('endsTo').value = val15;
                            ...
                            <table id="example" class="table table-striped responsive-utilities jambo_table" style="overflow-y:scroll;">
                                <thead>
                                    <tr class="headings">                                                   
                                        ...
                                        <th>Flat NO</th>
                                        <th>Contract code</th>
                                        <th>Contract Starts From</th>
                                        <th>Contract Ends TO</th>
                                    </tr>
                                </thead>
                                ...
                                <tbody>                                   
                                    ...
                                    <c:forEach items="${selectTenant}" var="type">
                                        ...
                                        <tr onclick="test('${type.tenantCode}', '${type.tenantFname} ${type.tenantLname}', '${type.assetNames}', '${type.buildingNo}', '${type.blockNo}', '${type.streetNo}', '${type.plotNo}', '${type.totalAmount}', '${type.billingCycle}', '${type.subclassifiedAstno}', '${type.flatNo}', '${type.assetName}', '${type.contractName}', '<fmt:formatDate pattern="dd-MMM-yyyy" value="${type.startsFrom}"/>', '<fmt:formatDate pattern="dd-MMM-yyyy" value="${type.endsTo}"/>');">                                            
                                            ...
                                            <td class="center" style="text-align: right;">${type.contractName}</td>
                                            <td class="center" style="text-align: right;">${type.startsFrom}</td> 
                                            <td class="center" style="text-align: right;">${type.endsTo}</td> 
                                        </tr>
                                    </c:forEach>
